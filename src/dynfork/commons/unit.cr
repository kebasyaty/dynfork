  ...
  def unit_manager(
    ...
    choices : Array(Tuple(Float64 | Int64 | String, String)) = (
      if dyn_field_type.includes?("Text")
        ...
        val_exists? = (choices_text.map { |item| item[0] }).includes?(unit.value)
        ...
        val_exists? = (choices_i64.map { |item| item[0] }).includes?(unit.value)
        ...
        val_exists? = (choices_f64.map { |item| item[0] }).includes?(unit.value)
